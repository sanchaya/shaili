...
const letterTagReducer = (tags: ITags, action: Action): ITags => {
  switch (action.type) {
    ...
    case "ADD_TAG":
      const newTaggedLetters = [...tags.taggedLetters, action.payload.tag];

      newTaggedLetters.sort(sortByUpdatedAt);
    ...
    case "REMOVE_TAG":
      ...
      filteredTaggedLetters.sort(sortByUpdatedAt);
    ...
    case "UPDATE_TAG":
      ...
      updatedTaggedLetters.sort(sortByUpdatedAt);
...
const LetterTagProvider = ({ children }: Props) => {
  ...
  const fetchTag = async (bookId: number) => {
    ...
    if (response.data) {
      ...
      const all = response.data.sort(sortByUpdatedAt);
      const recent = recentTags.sort(sortByUpdatedAt);
