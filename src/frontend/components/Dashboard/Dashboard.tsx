...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${BASE_URL}/get-tagged-letters?user=` + Number(currentAdmin?.id))
      .then((response) => {
    ...
    <Box style={{ width: "96%", margin: "0 auto" }}>
      ...
      <DashboardContainer>
        <DashboardItem>
          ...
          <Count>{totalBooks?.length}</Count>
        </DashboardItem>
        <DashboardItem>
          <CardTitle>Number of books in progress</CardTitle>
          <Count>
            {totalBooks?.filter((item) => item.status === 2).length}
          </Count>
        </DashboardItem>
        <DashboardItem>
          <CardTitle>Completed Books</CardTitle>
          <Count>
            {totalBooks?.filter((item) => item.status === 4).length}
          </Count>
        </DashboardItem>
      </DashboardContainer>
      <Box>
        <Box width={1 / 2} backgroundColor="#fff" borderRadius="10px">
          <h3
            style={{
              fontSize: "20px",
              padding: "25px",
