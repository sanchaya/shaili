...
const AddLetter = () => {
  ...
  const getLetterTypes = () => {
    axios.get(`${BASE_URL}/get-lettertypes`).then((response) => {
      let letterTypes: Record<string, { value: string; label: string }[]> = {};
      response.data.forEach(
        (letterType: { id: number; type: string; language: string }) => {
        ...
        {letterTypeOptions && (
          <FormGroup>
            ...
            <Select
              ...
              placeholder={
                language ? "Select a letter type" : "Select a language first"
              }
              required
            />
          </FormGroup>
