...
const EditLetter = (props) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [languagesResponse, letterTypesResponse] = await Promise.all([
        ...
        if (record.params?.letter_type) {
          const selectedLetterType = letterTypes[
            ...
            (option) => option.value === record.params.letter_type.toString()
          );

          if (selectedLetterType) {
            setLetterType(selectedLetterType);
          } else {
            console.warn("Letter type not found for record:", record);
          }
        }
      } catch (error) {
        addNotice({
          ...
          <Box>
            <FormGroup>
              ...
              <Input
                id="letter"
                name="letter"
                value={!lettername ? record.params.letter : lettername}
            ...
            {letterTypeOptions && (
              <FormGroup>
                ...
                <Select
                  ...
                  isClearable={false}
                  value={letterType}
                  options={letterTypeOptions[language?.value || ""] || []}
